...
At rest on Home and on the eight service landings, `title-sequence-nav` remaps the nav tokens to the dark cinematic overlay. After 18px of scroll that class lifts and the themed frosted bar takes over. Resource, vertical, hub, and legal pages do not use the overlay.
...
- If changing `title-sequence-nav` or the scroll threshold, verify Home **and** a service landing at rest and after scroll (desktop and mobile).
