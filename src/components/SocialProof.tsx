...
const SocialProof = () => {
    ...
    return (
        <section className="py-20 bg-secondary/35 border-y border-border/50">
            <div className="container relative z-10 mx-auto px-6 md:px-12">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                    {stats.map((stat, index) => (
                        <div
                            key={index}
                            className="cafe-card flex flex-col items-center p-6 bg-card/85"
                        >
                            {stat.icon}
                            <span className="text-3xl md:text-4xl font-bold text-foreground mb-2">
                                <AnimatedCounter end={stat.value} suffix={stat.suffix} />
                            </span>
                            <span className="text-sm text-muted-foreground uppercase tracking-[0.16em]">
                                {t(stat.labelKey)}
                            </span>
                        </div>
                    ))}
