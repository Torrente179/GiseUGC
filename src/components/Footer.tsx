  ...
  return (
    ...
    >
      ...
      <div className="studio-container">
        <motion.div
          className="relative overflow-hidden rounded-[2rem] border border-foreground/10 bg-background/88 px-6 py-8 shadow-[0_32px_90px_-56px_hsl(var(--foreground)/0.7)] backdrop-blur-sm dark:bg-card/85 md:px-10 md:py-10 lg:px-12 lg:py-12"
        ...
        >
          <motion.div
            className="grid gap-10 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)] lg:gap-12"
            variants={staggerContainer(0.08, 0.08)}
          >
            <motion.div className="space-y-6" variants={blurRevealUp(16, 0.65)}>
              <div className="flex flex-wrap items-center gap-3">
                <p className="brand-logo text-[1.85rem] leading-none">Gisela.UGC</p>
                <span className="section-label">{t('footer.eyebrow')}</span>
              </div>

              <div className="space-y-4">
                <h2 className="max-w-3xl text-[clamp(2.4rem,5vw,4.75rem)] leading-[0.92] text-balance">
                  {t('footer.title')}
                </h2>
                <p className="strategic-body max-w-2xl text-base text-foreground/72 md:text-lg">
              ...
              <div className="flex flex-wrap gap-2.5">
                {proofItems.map((item) => (
                  <span
                    key={item}
                    className="inline-flex items-center rounded-full border border-foreground/10 bg-background/76 px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-foreground/68 dark:bg-card/72"
                  >
                    {item}
                  </span>
                ))}
              ...
              <p className="max-w-xl text-sm leading-[1.85] text-foreground/62 md:text-[15px]">
                {t('footer.studioSummary')}
              </p>
            </motion.div>

            <motion.div
              className="rounded-[1.75rem] border border-foreground/10 bg-gradient-to-b from-background/96 via-background/92 to-secondary/30 p-6 shadow-[0_28px_70px_-48px_hsl(var(--foreground)/0.55)] dark:from-card/96 dark:via-card/92 dark:to-secondary/20 md:p-7"
              variants={blurRevealUp(18, 0.68)}
            >
              <p className="section-label mb-4">{t('footer.contactCardEyebrow')}</p>
              <h3 className="text-[clamp(1.7rem,2.4vw,2.35rem)] leading-[1.02] text-foreground">
                {t('footer.contactCardTitle')}
              </h3>
              <p className="mt-4 strategic-body text-sm text-foreground/72 md:text-[15px]">
              ...
              <div className="mt-6 flex flex-col gap-3">
                <motion.a
                  ...
                  className="group inline-flex min-h-12 items-center justify-between rounded-full bg-primary px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-primary-foreground shadow-[0_20px_45px_-28px_hsl(var(--primary)/0.95)] transition-all hover:-translate-y-[1px] hover:bg-primary/92"
                ...
                <motion.a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group inline-flex min-h-12 items-center justify-between rounded-full border border-foreground/12 bg-background/92 px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-foreground transition-colors hover:border-primary/35 hover:text-primary dark:bg-card/92"
                  whileHover={shouldReduceMotion ? undefined : { y: -2 }}
                  whileTap={shouldReduceMotion ? undefined : { scale: 0.985 }}
                  transition={springSmooth}
                >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="mt-5 inline-flex items-center gap-2 text-sm text-foreground/66 transition-colors hover:text-primary"
              >
          ...
          <motion.div
            className="mt-10 grid gap-8 border-t border-foreground/10 pt-8 md:grid-cols-3"
            variants={staggerContainer(0.05, 0.06)}
          >
            <motion.div variants={blurRevealUp(14, 0.45)}>
              ...
              <ul className="space-y-3">
                {serviceLinks.map((link) => (
                  <li key={link.href}>
                    <a
                      href={link.href}
                      className="group inline-flex items-center gap-2 text-sm text-foreground/72 transition-colors hover:text-primary md:text-[15px]"
                    >
                      <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-0.5" />
            ...
            <motion.div variants={blurRevealUp(14, 0.5)}>
            ...
            <motion.div variants={blurRevealUp(14, 0.55)}>
              ...
              <div className="flex flex-wrap gap-2.5">
                {profilePlatforms.map((platform) => (
                  <motion.a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex min-h-11 items-center gap-2 rounded-full border border-foreground/10 bg-background/82 px-4 py-2 text-sm text-foreground/68 transition-colors hover:border-primary/30 hover:text-primary dark:bg-card/80"
                    whileHover={shouldReduceMotion ? undefined : { y: -2 }}
                    whileTap={shouldReduceMotion ? undefined : { scale: 0.985 }}
                    transition={springSmooth}
                  >
